...
public class DeletionSwipeHelper extends ItemTouchHelper.SimpleCallback
{
    ...
    public DeletionSwipeHelper(int dragDirs, int swipeDirs, Context context, OnSwipeListener listener)
    {
        ...
        backgroundColor = ContextCompat.getColor(context, R.color.colorGreen);
        deleteColor = ContextCompat.getColor(context, R.color.colorPrimaryDark);
    ...
    public void onChildDraw(Canvas canvas, RecyclerView recyclerView, RecyclerView.ViewHolder viewHolder, float dX, float dY, int actionState, boolean isCurrentlyActive)
    {
        ...
        deleteIcon = ContextCompat.getDrawable(context, R.drawable.back);
        ...
        if (deleteIcon != null)
        {
            ...
            deleteIcon.setColorFilter(new PorterDuffColorFilter(ContextCompat.getColor(context, R.color.colorGreen), PorterDuff.Mode.SRC_IN));
            if (circleRadius > 0f)
            {
                canvas.drawCircle(cx, cy, circleRadius, circlePaint);
                deleteIcon.setColorFilter(new PorterDuffColorFilter(ContextCompat.getColor(context, R.color.colorAccent), PorterDuff.Mode.SRC_ATOP));
            }
